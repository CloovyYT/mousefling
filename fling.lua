...
function makepart(hat, part)
    ...
    create(
        workspace[LocalPlayer.Name][hat].Handle,
        workspace[LocalPlayer.Name]["lerp" .. hat],
        Vector3.new(),
        Vector3.new()
    )
    ...
    workspace[LocalPlayer.Name]["lerp" .. hat].Weld.C0 =
        workspace[LocalPlayer.Name]["lerp" .. hat].CFrame:inverse() * workspace[LocalPlayer.Name][part].CFrame *
        CFrame.new(0, 0, 0) *
        ...
        function()
            while true do
                ...
                workspace[game.Players.LocalPlayer.Name]["lerp" .. hat].Weld.C0 =
                    workspace[game.Players.LocalPlayer.Name]["lerp" .. hat].Weld.C0:lerp(
                    CFrame.new(
                        ppx + lppx * math.sin(hatsine / 11),
                        ppy + lppy * math.sin(hatsine / 11),
                        pppz + lpppz * math.sin(hatsine / 11)
                    ) *
                        CFrame.Angles(
                            math.rad(rrrx + lrrrx * math.sin(hatsine / 11)),
                            math.rad(rrry + lrrry * math.sin(hatsine / 11)),
                            math.rad(rrz + lrrz * math.sin(hatsine / 11))
                        ),
                    0.1
                )
                eswait()
...
local Scale =
    ...
    (game.Players.LocalPlayer.Character.Torso:FindFirstChild("ScaleInserted") ~= nil and
...
PartWeld.Part1 = Part
PartWeld.C0 =
    CFrame.Angles(math.rad(15), 0, 0) * CFrame.new(Vector3.new(0, 0.25, -0.75) * Scale) *
...
for i = 0, 80, 10 do
    ...
    PartWeld.Part1 = Part
    PartWeld.C0 =
        CFrame.new(Vector3.new(0, 0.15, 0.315) * Scale) * CFrame.Angles(math.rad(i - 65), 0, 0) *
        CFrame.new(Vector3.new(0, 0.2, 0) * Scale)
    PartWeld.Parent = Part
end
...
for i = 1, 75, 15 do
    ...
    PartWeld.Part1 = Part
    PartWeld.C0 =
        CFrame.new(Vector3.new(0, 0.525, -0.515) * Scale) * CFrame.Angles(math.rad(i), 0, 0) *
        CFrame.new(Vector3.new(0, 0, 0.2) * Scale)
    PartWeld.Parent = Part
end
...
function ShootBullet(Target, barrel)
    ...
    Bullet.CFrame =
        CFrame.new((barrel.CFrame * CFrame.new(0, 0, -barrel.Size.Z * barrel.Mesh.Scale.Z / 2)).p, Target) *
...
--Mesh.TextureId = "http://www.roblox.com/asset/?id=2697544"]]
    ...
    for i = Speed, 1000, Speed do -- Loop to do the bullet movement and stuff.
        ...
        ray =
            Ray.new(
            Bullet.Position,
            ((Bullet.CFrame * CFrame.Angles(math.rad(-Gravity), 0, 0) * CFrame.new(0, 0, -Speed)).p - Bullet.Position).unit *
                Speed
        )
        ...
        if Hit ~= nil then
            local Hum =
                Hit.Parent:FindFirstChild("Humanoid") or Hit.Parent.Parent:FindFirstChild("Humanoid") or
                (Hit.Parent.Parent.Parent ~= nil and Hit.Parent.Parent.Parent:FindFirstChild("Humanoid"))
            if Hum ~= nil then
...
game:GetService("RunService"):BindToRenderStep(
    ...
    function()
        ...
        if Point.Z > 0 then
            if Point.X > 0 then
                Torso.CFrame =
                    CFrame.new(Torso.Position, Vector3.new(Mouse.Hit.X, Torso.Position.Y, Mouse.Hit.Z)) *
        ...
        local CFr = (Torso.CFrame * Part0Joint):toObjectSpace(CFrame.new((Torso.CFrame * Part0Joint).p, Mouse.Hit.p))
         --RayEnd))
        ...
        Weld.Part1 = RA
        local CFr =
            (Torso.CFrame * Part0JointHead):toObjectSpace(CFrame.new((Torso.CFrame * Part0JointHead).p, Mouse.Hit.p))
         --RayEnd))
